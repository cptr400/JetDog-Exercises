 ...
 * @author Krish Pillai
 */
public class Exercise5F extends GameController {
	 ...
	 */
	public Exercise5F() {
		setTitle("Exercise 5F");
	}

	/**
	 * @param rate
	 */
	public Exercise5F(int rate) {
		super(rate);
	}
	 ...
	 * @param size
	 */
	public Exercise5F(int rate, Dimension size) {
		super(rate, size);
		...
		for (MessageEvent event : events) {
			...
			if (em instanceof AlienSoldier) {
				if (message.equals("fire")) {
					Fireball fireball = new Fireball(em.getXLocation(), em.getYLocation() + 30);
					addEntity(fireball);
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(() -> new Exercise5F());
	}
